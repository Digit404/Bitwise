...
function Install-IrisAndFabric {
    ...
    if ($confirm -eq "y" -or $confirm -eq "Y") {
        ...
        $irisURL = "https://github.com/IrisShaders/Iris-Installer/releases/download/3.0.2/Iris-Installer-3.0.2.jar"
        Invoke-WebRequest $irisURL -OutFile $irisJar
...
function Download-TexturePack {
    $confirm = Read-Host "Install a cool-ass texture pack with quality of life improvements? Optional (y/n)"
    ...
    if ($confirm -eq "y" -or $confirm -eq "Y") {
        ...
        $texturePackURL = "https://cdn.discordapp.com/attachments/1061066185778667661/1137873427965939732/VanillaTweaks_1.20.zip"
        Invoke-WebRequest $texturePackURL -OutFile $(Join-Path $resourcepacksPath $vanillaTweaks)
    }
}
...
function Copy-Options {
    $confirm = Read-Host "Copy options from the main profile? (y/n)"
    if ($confirm -eq "y" -or $confirm -eq "Y") {
...
function Setup-Installation {
    ...
    if (Test-Path $minecraftLancher) {
        Write-Host "Launching Minecraft Launcher..."
        Start-Process $minecraftLancher
    }
    else {
        Write-Host "Unable to find Minecraft Launcher, please launch it."
    }

    Write-Host "1. Create a new installation"
